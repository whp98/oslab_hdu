...
#define END 65535  	// 中的文件结束标志；FAT16中最多挂你了65536个簇
#define FREE 0  	// FAT中盘块空闲标志
#define ROOTBLOCKNUM 2 	// 根目录区所占盘块数
#define MAXOPENFILE 10 	// 最多同时打开文件个数t
#define MAXTEXT 10000	//改小后显示出现紊乱/************************************************************问题1 
...
/* 文件分配表 */
/*为了实现文件的链式存储,硬盘上必须准确地记录哪些簇已经被文件占用,
还必须为每个已经占用的簇指明存储后继内容的下一个簇的簇号,对一个文件的最后一簇,
则要指明本簇无后继簇。这些都是由FAT表来保存的,表中有很多表项,每项记录一个簇的信息。*/ 
...
void startsys()
{
	...
	//将myvhard中前SIZE个字节用 0 替换并返回 myvhard
    //memset()的函数， 它可以一字节一字节地把整个数组设置为一个指定的值,
	//它把数组的起始地址作为其第一个参数，第二个参数是设置数组每个字节的值，第三个参数是数组的长度(字节数，不是元素个数)。
    if((fp = fopen(myfilename, "r")) != NULL)//在当前目录下打开文件myfilename,只允许进行"读"操作，并使fp指向该文件
    {
...
int my_write(int fd)
{
    ...
    ll = 0;
    printf("please input write data(end with qaz):\n");

    //while(gets(text))
    while(1)
    {
	// input data store in 'text'
	gets(text);
	if(strcmp(text,"qaz")==0)
	    break;
        len = strlen(text);
...
int main()
{
    char cmd[15][10] = {"cd", "mkdir", "rmdir", "ls", "create", "rm", "open", "close", "write", "read", "exit","fat"};
    char s[30], *sp;
	int cmdn, flag = 1, i;
    startsys();
    printf("*********************欢迎来到FAT文件系统*****************************\n\n");
